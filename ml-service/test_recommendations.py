...
try:
    ...
    if response.status_code == 200:
        data = response.json()
        if data.get('success'):
            print(f"[OK] KNN Working! Found {data['count']} recommendations")
...
try:
    ...
    if response.status_code == 200:
        data = response.json()
        if data.get('success'):
            print(f"[OK] CF Working! Found {data['count']} recommendations")
            for i, rec in enumerate(data['recommendations'], 1):
                print(f"{i}. Event ID: {rec['event_id']}")
                print(f"   Similarity: {rec['similarity']*100:.1f}%")
                print()
...
try:
    ...
    if response.status_code == 200:
        data = response.json()
        if data.get('success'):
            print(f"[OK] Hybrid Working! Found {data['count']} recommendations")
