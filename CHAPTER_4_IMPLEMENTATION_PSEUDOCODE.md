...
Output: Authenticated Request or Access Denied

1: function VALIDATEJWTTOKEN(request)
2:    token ← extractTokenFromHeader(request.headers.Authorization)
3:    if token = null then
4:        return AccessDenied("No token provided")
5:    end if
6:    try
7:        decoded ← jwt.verify(token, JWT_SECRET)
8:        request.user ← decoded
9:        return ContinueToNextMiddleware()
10:   catch InvalidTokenError
11:       return AccessDenied("Invalid token")
12:   end try
13: end function
...
Input: User Role, List of Allowed Roles
Output: Access Granted or Access Denied

1: function VALIDATEUSERROLE(userRole, allowedRoles)
2:    if userRole ∈ allowedRoles then
3:        return AccessGranted()
4:    else
5:        return AccessDenied("Insufficient privileges")
6:    end if
7: end function
...
Input: Uploaded File, Size Limit, Allowed MIME Types
Output: File Path or Upload Error

1: function VALIDATEFILEUPLOAD(file, sizeLimit, allowedTypes)
2:    if file.size > sizeLimit then
3:        return UploadError("File size exceeds limit")
4:    end if
5:    if file.mimeType ∉ allowedTypes then
6:        return UploadError("Invalid file type")
7:    end if
8:    uniqueFilename ← generateUniqueFilename(file.originalName)
9:    filePath ← saveFile(file, uniqueFilename)
10:   return filePath
11: end function
...
Input: Event States and Transition Rules
Output: State Machine Configuration

1: function DEFINESTATEMACHINE()
2:    states ← {
3:        DRAFT: "Draft",
4:        SUBMITTED: "Submitted", 
5:        UNDER_REVIEW: "Under Review",
6:        TREASURER_APPROVED: "Approved by Treasurer",
7:        GENSEC_APPROVED: "Approved by Gen Sec",
8:        CHAIRPERSON_APPROVED: "Approved by Chairperson",
...
13:   transitions ← {
14:       DRAFT: [SUBMITTED],
15:       SUBMITTED: [UNDER_REVIEW, REJECTED],
16:       UNDER_REVIEW: [TREASURER_APPROVED, REJECTED],
17:       TREASURER_APPROVED: [GENSEC_APPROVED, REJECTED],
18:       GENSEC_APPROVED: [CHAIRPERSON_APPROVED, REJECTED],
19:       CHAIRPERSON_APPROVED: [PUBLISHED, REJECTED],
20:       PUBLISHED: [CANCELLED],
21:       REJECTED: [SUBMITTED],
22:       CANCELLED: []
23:   }
24:   return {states, transitions}
25: end function
...
1: function NEEDSVOLUNTEERS(event)
2:    return event.requirements.volunteers_needed > 0
3: end function

4: function NEEDSVENUE(event)
5:    return event.requirements.rooms_needed > 0
6: end function

7: function NEEDSREFRESHMENTS(event)
8:    return event.requirements.refreshments_needed = true
9: end function

10: function ISHIGHPRIORITY(event)
11:    daysUntil ← (event.date - currentDate) / (24 * 60 * 60 * 1000)
12:    return daysUntil ≤ 3 OR event.expected_participants > 200 OR event.prize_pool > 50000
13: end function
...
1: function CALCULATEEVENTBUDGET(eventId)
2:    requirements ← EventRequirement.findAll({event_id: eventId})
3:    mappedCosts ← requirements.map(r → Number(r.estimated_cost || 0))
4:    totalBudget ← mappedCosts.reduce((sum, cost) → sum + cost, 0)
...
Input: Payment Order ID, Payment ID, Signature
Output: Verification Result

1: function VERIFYPAYMENT(orderId, paymentId, signature)
2:    body ← orderId + "|" + paymentId
3:    expectedSignature ← HMAC_SHA256(body, RAZORPAY_SECRET)
4:    if expectedSignature = signature then
5:        updateRegistrationStatus(paymentId, "VERIFICATION_PENDING")
6:        return PaymentVerified()
7:    else
8:        return PaymentRejected("Invalid signature")
9:    end if
10: end function
...
Input: User Vector, Top K Recommendations
Output: Recommended Events with Similarity Scores

1: function USERBASEDCF(userVector, topK)
2:    userSimilarities ← cosineSimilarity([userVector], interactionMatrix)[0]
3:    similarUsers ← argsort(userSimilarities, descending=true)[0:10]
4:    
5:    recommendations ← zeros(datasetLength)
6:    for each userIdx in similarUsers do
7:        if userSimilarities[userIdx] > 0.1 then
8:            recommendations += interactionMatrix[userIdx] × userSimilarities[userIdx]
9:        end if
10:   end for
11:   
12:   topEvents ← argsort(recommendations, descending=true)[0:topK]
13:   return topEvents
14: end function
...
1: function RAGPIPELINE(query)
2:    embeddingModel ← SentenceTransformer('all-MiniLM-L6-v2')
3:    chromaClient ← ChromaDB.PersistentClient("./vectorstore")
4:    collection ← chromaClient.getCollection("niral_knowledge")
5:    
6:    queryResults ← collection.query(query_texts=[query], n_results=5)
7:    contexts ← extractContexts(queryResults)
8:    
9:    return contexts
10: end function
...
Input: PDF Template, Participant Data, Event Data
Output: Personalized Certificate

1: function PROCESSCERTIFICATETEMPLATE(template, participant, event)
2:    pdfDocument ← loadPDFTemplate(template)
3:    formFields ← pdfDocument.getFormFields()
4:    
5:    formFields['participant_name'] ← participant.name
6:    formFields['college_name'] ← participant.college
7:    formFields['event_name'] ← event.title
8:    formFields['event_date'] ← formatDate(event.date)
9:    
10:   pdfDocument.flattenForm()
11:   certificateBytes ← pdfDocument.save()
12:   
13:   return certificateBytes
14: end function
...
Input: Event Object, State Transition
Output: Role-Specific Notifications

1: function SETUPNOTIFICATIONHANDLERS()
2:    handlers ← {}
3:    handlers["SUBMITTED → UNDER_REVIEW"] ← () → notifyRole("TREASURER", "New event submitted for budget review")
4:    handlers["TREASURER_APPROVED"] ← () → notifyRole("GENERAL_SECRETARY", "Event approved by Treasurer")
5:    handlers["CHAIRPERSON_APPROVED → PUBLISHED"] ← () → {
6:        notifyRole("EVENT_TEAM", "Event published successfully")
7:        notifyRole("STUDENT", "New event available for registration")
8:    }
9:    return handlers
10: end function
...
Input: User Context with Role Information
Output: Appropriate Dashboard Component

1: function EVENTSROUTER(userContext)
2:    user ← userContext.user
3:    
4:    if user = null then
5:        return <Navigate to="/login" />
6:    end if
7:    
8:    roleComponentMap ← {
9:        "ADMIN": <AdminEventsPage />,
10:       "EVENT_TEAM": <EventTeamEventsPage />,
11:       "TREASURER": <TreasurerEventsPage />,
12:       "GENERAL_SECRETARY": <GeneralSecretaryEventsPage />,
13:       "CHAIRPERSON": <ChairpersonEventsPage />,
14:       "LOGISTICS": <LogisticsEventsPage />,
15:       "HR": <HREventsPage />,
16:       "HOSPITALITY": <HospitalityEventsPage />,
17:       "TECHOPS": <TechopsEventsPage />,
18:       "STUDENT": <ParticipantHomePage />,
19:       "PARTICIPANT": <ParticipantHomePage />
20:   }
21:   
22:   return roleComponentMap[user.role] || <Navigate to="/login" />
23: end function
